...
int		ft_add_tetri(char *tetri_map, t_dlist *current, int step)
{
	...
	while (tetri_map[step] != '\0' && tetri_map[step] != '.')
		step++;
	if (step + coo[3] - coo[0] < map_len && tetri_map[step + coo[1] - coo[0]] == '.'
		&& tetri_map[step + coo[2] - coo[0]] == '.' && tetri_map[step + coo[3] - coo[0]] == '.')
	{
...
int		ft_solve(t_dlist *val_list, char *tetri_map, int step)
{
	...
	current = val_list;
	while (current != NULL || (current->next == NULL && step != -1)) // если у нас нет текущего как мы пытаемся его добавить? в каком случе будет второе или
	{
